...
class RayIPPOLearner():
    ...
    def cal_influence_intrinsic_reward(self, multi_turn_batchs, reward_tensor_all, metrics):
        """
        基于多轮 reward 的变化，给当前 agent 的 reward tensor 加额外惩罚 / 激励。
        reward_tensor: [bs, seq_len]，当前 agent 的 token 级奖励。
        返回值 shape 不变。
        """
        # 1. 读取配置
        # penalty_value = getattr(self.config.marl, "influence_penalty", 0.1)
        # tolerance = getattr(self.config.marl, "influence_tolerance", 1e-6)
        # reduce_mode = getattr(self.config.marl, "influence_reduce_mode", "mean")  # "mean" 或 "sum"

        # batch = agent_batchs[self.curr_agent_key]  # 需要在调用前设置 self.curr_agent_key
        # response_mask = batch.batch.get("response_mask")
        # if response_mask is None:
        #     # fallback：没有 mask 时，直接返回原始奖励
        #     return reward_tensor

        # 2. 当前 agent 每轮 reward（按 turn 内 token 平均）
        for turn_idx in range(self.turns-1):
            current_batch = multi_turn_batchs[f"turn_{turn_idx}"]

            # next_batch = agent_batchs[f"turn_{turn_idx+1}"]

            for agent_id in range(self.num_agents):
                current_reward = reward_tensor_all[f"turn_{turn_idx}"][f"agent_{agent_id}"]
                next_reward = reward_tensor_all[f"turn_{turn_idx+1}"][f"agent_{agent_id}"]


                delta_reward = next_reward.sum(-1) - current_reward.sum(-1)
                intrinsic_reward = torch.sign(delta_reward) * 0.2

                bs = current_reward.shape[0]
                device = current_reward.device
                
                # 方法1: 通过 response_mask 找到最后一个有效 token 位置
                response_mask =  current_batch[f"agent_{agent_id}"].batch["response_mask"]

                if response_mask is not None:
                    # 计算每个样本的最后一个有效 token 位置
                    last_valid_positions = response_mask.sum(-1) - 1  # [bs]
                    # 将 intrinsic_reward 加到对应位置
                    batch_indices = torch.arange(bs, device=device)
                    current_reward[batch_indices, last_valid_positions] += intrinsic_reward
                else:
                    # 方法2: 如果没有 mask，通过 nonzero 找到非0位置
                    for i in range(bs):
                        nonzero_pos = torch.nonzero(current_reward[i], as_tuple=False)
                        if len(nonzero_pos) > 0:
                            # 取最后一个非0位置
                            pos = nonzero_pos[-1].item()
                            current_reward[i, pos] += intrinsic_reward[i]
                
                # 更新 reward_tensor
                reward_tensor_all[f"turn_{turn_idx}"][f"agent_{agent_id}"] = current_reward


                # 记录指标
                metrics[f"influence/turn_{turn_idx}-agent_{agent_id}"] = intrinsic_reward.mean().item()
    ...
    def _compute_adv(self, agent_batchs, estimate_reward_tensor_all, reward_extra_infos_dict_all, future_reward_all, metrics):
        ...
        #     estimate_reward_tensor_all = self.cal_influence_intrinsic_reward(agent_batchs, estimate_reward_tensor_all, metrics)
