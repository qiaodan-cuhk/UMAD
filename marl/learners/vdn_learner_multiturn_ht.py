...
class RayVDNLearner_MultiTurn_Hetero(RayMARLLearner):
    def __init__(self,
                 config,
                 mac,
        ...
        #### todo: (qd) 需要修改ray marl learner hetero类，暂时这里先不改
        agent_tokenizer = tokenizer_list[0]
        agent_processor = processor_list[0]


        #### todo：删除 reward fn，learner用不到
    ...
    def _compute_vdn_values(self, agent_batchs, q_total_clipbase, metrics, union=True):
        ...
        #### todo: (qd) 处理不同mask token长度的q total计算
        ...
        all_agent_attn_masks = []
        
        # 遍历所有agent，找到最大response长度并准备数据
        for agent_key, agent_batch_data in agent_batchs.items():
            if isinstance(agent_batch_data, DataProto):
                processed_data = {**agent_batch_data.batch.to(get_torch_device().current_device()), 
                                **agent_batch_data.non_tensor_batch}
            else:
                processed_data = agent_batch_data.to(get_torch_device().current_device())
            
            # responses = processed_data["responses"]
            attention_mask = processed_data["attention_mask"]
            # all_agent_response[f'{agent_key}'] = responses
            all_agent_attn_masks.append(attention_mask)

            

        stacked_all_attn_masks = torch.stack(all_agent_attn_masks, dim=0)  # [num_agents, bs, response_len]
        if union:
            # 取max：任何一个agent在该位置有有效token，联合mask就有效
            union_attention_mask = torch.max(stacked_all_attn_masks, dim=0)[0]  # [batch_size, seq_length]
        else:
            # 取min：所有agent在该位置都有有效token，联合mask才有效  
            union_attention_mask = torch.min(stacked_all_attn_masks, dim=0)[0]  # [batch_size, seq_length]

        # 任意agent的responses长度都一样
        team_returns = processed_data["returns"]
        response_length = processed_data["responses"].size(1)
        response_mask = union_attention_mask[:, -response_length - 1 : -1]
