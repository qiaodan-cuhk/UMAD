...
# Optional async rollout support from newer verl versions.
from verl.workers.rollout.async_server import AsyncLLMServerManager

# MARL helpers map local PPO worker names back to global MARL roles.
...
"""Resource pool manager with per-agent MARL role mapping."""
@dataclass
class ResourcePoolManager:
    ...
    def get_resource_pool(self, agent_id: int, role: Role) -> RayResourcePool:
        ...
        if agent_id is not None:
            # Convert a local PPO role into the corresponding global MARL role.
            role_conversion = {
    ...
    """Return a global worker pool, for example an embedding worker pool."""
...
# Per-agent PPO wrapper used by the MARL controller.
class RayPPOAgent(object):
    """
    ...
    def __init__(self,
                 ...
                 reward_fn=None,
                 val_reward_fn=None,
                 unthinking_mode=False,  # Disable Qwen3-style thinking when supported.
                 **kwargs):
        ...
        # The top-level trainer validates and passes shared runtime flags.
        ...
        # The top-level trainer computes these flags across all MARL agents.
        ...
        if self.use_reference_policy:
            if config.algorithm.kl_ctrl.type == 'fixed':
                ...
                assert config.algorithm.kl_ctrl.horizon > 0, f'horizon must be larger than 0. Got {config.algorithm.kl_ctrl.horizon}'
        ...
        # Agent configs are already specialized by the controller when needed.
    ...
    def init_workers(self):
        ...
        """Create worker groups that belong to this agent's mapped roles."""
        ...
        """Optional async rollout manager from newer verl versions."""
    ...
    def _save_checkpoint(self, local_path, remote_path=None, global_steps=0, max_actor_ckpt_to_keep=None, max_critic_ckpt_to_keep=None):
        """Save one agent's checkpoint."""
        print(f"Saving checkpoint for agent_{self.agent_id}")
        
        # Save actor.
        actor_local_path = os.path.join(local_path, "actor")
        actor_remote_path = os.path.join(remote_path, "actor") if remote_path else None
        ...
        # Save critic when enabled.
        if self.use_critic and hasattr(self, 'critic_wg'):
            critic_local_path = os.path.join(local_path, "critic")
            critic_remote_path = os.path.join(remote_path, "critic") if remote_path else None
        ...
        print(f"Successfully saved checkpoint for agent_{self.agent_id}")


    def _load_checkpoint(self, local_path, del_local_after_load=True):
        """Load one agent's checkpoint."""
        print(f"Loading checkpoint for agent_{self.agent_id}")
        
        actor_path = os.path.join(local_path, "actor")
        critic_path = os.path.join(local_path, "critic")
        ...
        # Load critic when enabled.
        if self.use_critic and hasattr(self, 'critic_wg'):
            if not os.path.exists(critic_path):
                print(f"Warning: Critic checkpoint not found at {critic_path}, skipping")
            else:
        ...
        print(f"Successfully loaded checkpoint for agent_{self.agent_id}")
