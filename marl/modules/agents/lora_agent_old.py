...
class RayLoraAgent(object):
    ...
    """考虑保留在PPO层级，marl使用agent._validate调用"""
    def _validate(self):
        reward_tensor_lst = []
        data_source_lst = []
    ...
    # # 拆分成两部分功能，register和setup，用于marl层统一分配地址
    # def register_worker_classes(self, global_resource_pool_to_cls):
    #     """注册该agent需要的worker classes到全局映射"""
    #     if self.hybrid_engine:
    #         resource_pool = self.resource_pool_manager.get_resource_pool(self.agent_id, Role.ActorRollout)
    #         # 使用agent_id前缀注册
    #         worker_name = f'agent_{self.agent_id}_actor_rollout'
    #         global_resource_pool_to_cls[resource_pool][worker_name] = RayClassWithInitArgs(
    #             cls=self.role_worker_mapping[Role.ActorRollout],
    #             config=self.config.actor_rollout_ref,
    #             role='actor_rollout'    # 这里要检查为什么有role，如何影响
    #         )
    ...
    #         # 使用agent_id前缀注册
    #         worker_name = f'agent_{self.agent_id}_critic'
    #         global_resource_pool_to_cls[resource_pool][worker_name] = RayClassWithInitArgs(
    ...
    #         # 使用agent_id前缀注册
    #         worker_name = f'agent_{self.agent_id}_ref'
    #         global_resource_pool_to_cls[resource_pool][worker_name] = RayClassWithInitArgs(
    ...
    #         # 使用agent_id前缀注册
    #         worker_name = f'agent_{self.agent_id}_reward_model' 
    #         global_resource_pool_to_cls[resource_pool][worker_name] = RayClassWithInitArgs(
    #             cls=self.role_worker_mapping[Role.RewardModel],
    #             config=self.config.reward_model
    #         )

    # def setup_worker_groups(self, global_worker_groups):
    #     """设置worker引用并初始化模型"""
    #     self.wg_dicts = []

    #     # 根据agent_id前缀获取属于自己的worker
    #     if self.use_critic:
    #         self.critic_wg = global_worker_groups[f'agent_{self.agent_id}_critic']
    #         self.critic_wg.init_model()  # 初始化LLM参数
        
    #     if self.use_reference_policy:
    #         self.ref_policy_wg = global_worker_groups[f'agent_{self.agent_id}_ref']
    #         self.ref_policy_wg.init_model()

    #     if self.use_rm:
    #         self.rm_wg = global_worker_groups[f'agent_{self.agent_id}_reward_model']
    #         self.rm_wg.init_model()

    #     # we should create rollout at the end so that vllm can have a better estimation of kv cache memory
    #     self.actor_rollout_wg = global_worker_groups[f'agent_{self.agent_id}_actor_rollout']
