...
# Allow launch scripts to select the Hydra config dynamically.
...
# legacy trainer comment retained for compatibility
...
class TaskRunner:
    ...
    def run_marl(self, config):
        ...
        # Each agent can use an independently configured model checkpoint.
        ...
        # FSDP is the primary supported backend for this release path.
        ...
        # Build per-agent role mappings consumed by the MARL controller.
        ...
        # All agents share a global multi-GPU resource pool.
        ...
        if config.reward_model.enable:
            ...
            # Attach reward-model workers for each agent when enabled.
            for agent_id in range(agent_nums):
                reward_model_role = MARLRole[f"agent_{agent_id}_RewardModel"]
                role_worker_mappings[agent_id][reward_model_role] = ray.remote(RewardModelWorker)
                mapping[reward_model_role] = global_pool_id
        ...
        # Heterogeneous agents use their own tokenizer/processor and reward manager.
        ...
        # The global resource pool manager owns the flattened MARL role mapping.
        ...
        trainer = trainer_cls(config=config,
                                ...
                                tokenizer_list=tokenizer_list,  # Per-agent tokenizers.
                                processor_list=processor_list,  # Per-agent processors.
                                ...
                                reward_fn_list=reward_fn_list,  # Per-agent reward functions.
