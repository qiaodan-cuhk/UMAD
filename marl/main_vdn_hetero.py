...
class TaskRunner:
    ...
    def run_marl(self, config):
        ...
        # # trainer_cls = Registry["marl_naive"]
        # trainer_cls = Registry[algorithm_name]
        ...
        # sum_rewards = os.getenv("SUM_REWARD", "false").lower() == "true"  # 读取环境变量并转换为布尔值
        # cory_flip = os.getenv("CORY_FLIP", "false").lower() == "true"  # 读取环境变量并转换为布尔值

        # common kwargs
        # 可以用config.marl形式调用了，不用kwargs传入
        # kwargs['sum_rewards'] = config.marl.sum_reward
        # kwargs['cory_flip'] = config.marl.cory_flip
        # kwargs['tb_dir'] = config.marl.tensorboard_dir

        from marl.marl_trainer import RayMARLTrainer
        trainer_cls = RayMARLTrainer
