...
class MARLRole(Enum):
    """Role keys used by the multi-agent ResourcePoolManager."""
    # Agent 0
    agent_0_ActorRollout = auto()
    agent_0_Critic = auto()
...
class Role(Enum):
    """
    Standard PPO role mapping used to route each worker type.
    """
    Actor = 0
    Rollout = 1
    ActorRollout = 2
...
def _convert_worker_name_to_marl_role_name(worker_name: str) -> str:
    """
    Convert a worker name to the suffix used in MARLRole enum names.
    
    Args:
        worker_name: Worker name such as "actor_rollout", "critic", or "ref".
        
    Returns:
        The corresponding MARLRole suffix.
    """
    name_mapping = {
...
def create_marl_dataset(data_paths, data_config, tokenizer_list, processor_list, full_config=None):
    ...
    # Read per-agent template controls from the full config.
    unthinking_mode = []
    agent_configs = None
...
def _convert_marl_to_ppo_roles(marl_role_mapping: dict, agent_id: int) -> dict:
    """
    Convert the global MARL role mapping into the PPO role mapping for one agent.
    
    Args:
        marl_role_mapping: Mapping that contains worker classes for all agents.
        agent_id: Current agent id.
        
    Returns:
        Standard PPO role mapping for the requested agent.
    """
    ppo_role_mapping = {}
...
class ResourcePoolManager:
    """
    Resource pool manager that resolves PPO roles back to per-agent MARL roles.
    """
    ...
    def get_resource_pool(self, agent_id: int, role: Role) -> RayResourcePool:
        ...
        if agent_id is not None:
            # Convert standard PPO roles to agent-specific MARL roles.
            role_conversion = {
