...
def collate_fn_marl(data_list: list[dict]) -> dict:
    """
    Collate a batch of per-agent sample dicts into batched tensors and arrays.

    Args:
        data_list: List of samples in the form
            {"agent_0": row_dict, "agent_1": row_dict, ...}.

    Returns:
        Dict keyed by agent id. Tensor entries are stacked into torch.Tensor
        batches and non-tensor entries are converted to object arrays.
    """
...
class MARLRLHFDataset(Dataset):
    """
    RLHF dataset for heterogeneous multi-agent training.

    Each agent can use a different tokenizer or processor while consuming the
    same underlying prompt records.
    """
    def __init__(
        self,
        ...
        unthinking_mode: Optional[List[bool]] = None,
        **kwargs
    ):
        ...
        self.sample_ratio = config.get("sample_ratio", 1.0)
    ...
    def check_length_for_all_agents(self, doc):
        """Return whether the prompt fits every agent tokenizer."""
    ...
    def __getitem__(self, item):
        """Build one shared prompt into per-agent model inputs."""
