...
class TaskRunner:
    ...
    def run_marl(self, config):
        ...
        # todo: 异构agents使用不同的tokenizer和processor，会影响dataset和dataloader的变化，目前暂时不修改，使用第一个agent的
        ...
        train_dataset = create_marl_dataset(config.data.train_files, config.data, tokenizer_list, processor_list)
        val_dataset = create_marl_dataset(config.data.val_files, config.data, tokenizer_list, processor_list)
        ...
        from marl.marl_trainer_multiturn_ht import RayMARLTrainer_MultiTurn_Hetero
        trainer_cls = RayMARLTrainer_MultiTurn_Hetero
