...
class RayMARLTrainer_MultiTurn:
    """
    ...
    def __init__(self,
        ...
        """vdn版本，目前只支持multi turn"""
        # learner = LEARNER_REGISTRY[config.marl.name]
        learner = LEARNER_REGISTRY["vdn_multiturn"]
        ...
        self.sentence_level_reward = 'mean'  # discount, counterfactual, learn
        # self.learn_sentence_q = False # 默认不学习sentence level的Q
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                for turn_idx in range(self.turns):
                    ...
                    for agent_id, agent in enumerate(self.mac.agents):
                        multi_turn_batchs[f"turn_{turn_idx}"][f"agent_{agent_id}"] = DataProto.from_single_dict(deepcopy(batch_dict))
                ...
                # single agent single turn版本
                # batch: DataProto = DataProto.from_single_dict(batch_dict)


                ######### 暂时只考虑使用同构model，且同类型model，不使用多模态和tool等方法 #########
