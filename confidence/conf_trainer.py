...
class ConfidenceTrainer:
    ...
    """计算原始reward, 而不是team reward，用于记录"""
    ...
    def _validate(self):
        ...
        # 所有batch处理完成后，记录index和各agent的reward
    ...
    # 在1036行之后添加保存逻辑
    def save_analysis_data_hdf5(self, multi_turn_batchs, reward_tensor_all, 
                                global_step, save_dir):
        """
        高效保存分析数据到HDF5格式
        
        优化：
        - log prob使用float16（节省50%空间，0.01级别精度足够）
        - token ids使用int32
        - 使用gzip压缩（level=1平衡速度和大小）
        - 变长文本单独存储
        """
        if not save_dir:
            return
        ...
        hdf5_path = os.path.join(save_dir, f"analysis_step_{global_step}.h5")
        
        with h5py.File(hdf5_path, 'w') as f:
            # 创建step组
            step_group = f.create_group(f"step_{global_step}")
            
            # 获取batch size
            first_agent_batch = multi_turn_batchs["turn_0"][f"agent_0"]
            batch_size = first_agent_batch.batch["responses"].size(0)
            
            # 存储元数据
            metadata_group = step_group.create_group("metadata")
            metadata_group.attrs["global_step"] = global_step
            metadata_group.attrs["num_samples"] = batch_size
            metadata_group.attrs["num_turns"] = self.turns
            metadata_group.attrs["num_agents"] = self.num_agents
            
            # 创建sample_ids
            sample_ids = [f"step_{global_step}_sample_{i}" for i in range(batch_size)]
            dt = h5py.string_dtype(encoding='utf-8')
            metadata_group.create_dataset("sample_ids", data=sample_ids, dtype=dt)
            
            # 处理每个turn
            for turn_idx in range(self.turns):
                agent_batchs = multi_turn_batchs[f"turn_{turn_idx}"]
                turn_group = step_group.create_group(f"turn_{turn_idx}")
                
                for agent_id, agent in enumerate(self.mac.agents):
                    agent_batch = agent_batchs[f"agent_{agent_id}"]
                    tokenizer = self.tokenizer_list[agent_id]
                    agent_group = turn_group.create_group(f"agent_{agent_id}")
                    # 获取数据
                    entropy = agent_batch.batch["entropys"]  # (bs, response_len)
                    # input_ids = agent_batch.batch["input_ids"]  # (bs, seq_len)
                    responses = agent_batch.batch["responses"]  # (bs, response_len)
                    response_mask = agent_batch.batch["response_mask"]  # (bs, response_len)
                    token_log_probs = agent_batch.batch["old_log_probs"]  # (bs, response_len)
                    # sentence_confidences = agent_batch.batch["confidences"]  # (bs,)

                    logV = torch.log(torch.tensor(tokenizer.vocab_size, device=entropy.device, dtype=entropy.dtype))
                    confidences_token = logV - entropy
                    confidences_sentence = confidences_token.mean(dim=-1)
                    prompts_ids = agent_batch.batch["prompts"]  # (bs, prompt_len)
                    responses_ids = agent_batch.batch["responses"]  # (bs, response_len)


                    # # 计算实际长度
                    # prompt_lengths = (prompts != tokenizer.pad_token_id).sum(dim=-1).cpu().numpy().astype(np.int32)
                    # response_lengths = response_mask.sum(dim=-1).cpu().numpy().astype(np.int32)
                    
                    # # 获取最大长度（用于padding）
                    # max_prompt_len = prompts.size(1)
                    max_response_len = responses.size(1)
                    
                    # # 转换为numpy并处理
                    # prompts_np = prompts.cpu().numpy().astype(np.int32)
                    # responses_np = responses.cpu().numpy().astype(np.int32)
                    # response_mask_np = response_mask.cpu().numpy().astype(np.bool_)
                    
                    # log prob转换为float16（节省空间）
                    token_log_probs_np = token_log_probs.cpu().numpy().astype(np.float16)
                    ...
                    agent_group.create_dataset(
                        ...
                        compression='gzip',
                        compression_opts=1,
                        chunks=(min(32, batch_size), prompts_ids.size(1))
                    )
                    ...
                    agent_group.create_dataset(
                        ...
                        compression='gzip',
                        compression_opts=1,
                        chunks=(min(32, batch_size), max_response_len)
                    )
                    
                    # 存储log prob（float16 + 压缩）
                    agent_group.create_dataset(
                        "token_log_probs",
                        data=token_log_probs_np,
                        dtype=np.float16,
                        compression='gzip',
                        compression_opts=1,
                        chunks=(min(32, batch_size), max_response_len)
                    ...
                    agent_group.create_dataset(
                        "entropy",
                        data=entropy.cpu().numpy().astype(np.float16),
                        dtype=np.float16,
                        compression='gzip',
                        compression_opts=1,
                        chunks=(min(32, batch_size), max_response_len)
                    ...
                    # 存储长度信息
                    agent_group.create_dataset("prompt_lengths", data=prompts_ids.size(1), dtype=np.int32)
                    agent_group.create_dataset("response_lengths", data=responses_ids.size(1), dtype=np.int32)
                    
                    # 存储sentence confidence（float32足够）
                    agent_group.create_dataset(
                        "confidences_sentence",
                        data=confidences_sentence.cpu().numpy().astype(np.float32),
                        dtype=np.float32
                    )
                    agent_group.create_dataset(
                        "confidences_token",
                        data=confidences_token.cpu().numpy().astype(np.float32),
                        dtype=np.float32
                    )
                    
                    # 存储reward，用 ind 模式rollout，这样每个人只记录自己的reward
                    agent_turn_reward = reward_tensor_all[f"turn_{turn_idx}"][f"agent_{agent_id}"].sum(-1).detach()
                    agent_group.create_dataset(
                        "reward",
                        data=agent_turn_reward.cpu().numpy().astype(np.float32),
                        dtype=np.float32
                    )
                    
                    # 存储文本（变长字符串，单独存储）
                    prompt_texts = tokenizer.batch_decode(prompts_ids, skip_special_tokens=True)
                    response_texts = tokenizer.batch_decode(responses_ids, skip_special_tokens=True)
                    
                    # HDF5字符串数组
                    dt = h5py.string_dtype(encoding='utf-8')
                    agent_group.create_dataset("prompt_texts", data=prompt_texts, dtype=dt)
                    agent_group.create_dataset("response_texts", data=response_texts, dtype=dt)
        
        print(f"Analysis data saved to {hdf5_path} (compressed HDF5 format)")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # === offpolicy，存储到buffer,考虑到需要next state去生成不同的next action，这里存储embed model的ids，但是也可以考虑存储文本 ===
                    ...
                    """
                    把所有的ref log prob reward update都放到了learner中，对于qmix额外传入buffer采样
                    需要考虑如何在update中支持策略采样
                    这里high level offpolicy QMIX从buffer学习sentence reward，low level onpolicy用multi turn batchs的Qi作为traj的reward
                    由于在learn过程中需要采样next state的next action，需要把embed ids转换成文本作为prompt，然后tokenize化并应用template，进入DP生成得到response，再转为embed action计算Q
                    """
                    # print("Start Training")
                    # with _timer("update", timing_raw):
                    #     self.learner.train(multi_turn_batchs, reward_tensor_all, reward_extra_infos_dict_all, future_reward_all, self.buffer, metrics, self.global_steps, timing_raw)



                    # rollout bs计算confidence和log prob
                    """每个turn、每个agent计算"""
                    for turn_idx in range(self.turns):
                        ...
                        for agent_id, agent in enumerate(self.mac.agents):
                            ...
                            # 计算log prob和confidence
                            old_log_prob = agent.actor_rollout_wg.compute_metrics_for_confidence(agent_batch)


                            entropys = old_log_prob.batch["entropys"]
                            agent_batch.batch["entropys"] = entropys.detach()
                            # confidence
                            confidences = old_log_prob.batch["confidences"]
                            # 记录每个token的log prob
                            token_log_prob = old_log_prob.batch["old_log_probs"]
                    ...
                    # analysis_data_dir = self.config.trainer.get("analysis_data_dir", None)

                    analysis_data_dir = "/root/code/maverl/confidence/analysis_data"
                    if analysis_data_dir:
                        analysis_data_dir = os.path.join(
                            analysis_data_dir,
                            self.config.trainer.get("experiment_name", "latest")
                        )
                        self.save_analysis_data_hdf5(
                            multi_turn_batchs,
                            reward_tensor_all,
                            self.global_steps,
                            analysis_data_dir
                    ...
                    if self.config.algorithm.adv_estimator == "grpo":
                        ...
                        if rollout_data_dir:
                            ...
                            # 处理全部的batch数据，免得采样对齐，记录estimated rewards，advantages，index，题型, 难度
                    ...
                    #     if self.config.marl.save_qmix_model:
                    #         qmix_ckpt_path = os.path.join(self.config.trainer.get("rollout_data_dir", None), self.config.trainer.get("experiment_name", "latest"), f"qmix_step_{self.global_steps}.pt")
                    #         self.learner.save_qmix_network(qmix_ckpt_path)
                ...
                """
                # collect metrics
                if self.buffer.size > self.config.marl.train_start_size:
                    if self.learner.qmix_update_count >= self.config.marl.qmix_critic_warmup:
