...
if [ $USE_SGLANG -eq 1 ]; then
    pip install -i $PIP_INDEX_URL "sglang[all]==0.4.6.post1" --no-cache-dir --find-links https://flashinfer.ai/whl/cu124/torch2.6/flashinfer-python && pip install torch-memory-saver --no-cache-dir
fi
pip install -i $PIP_INDEX_URL --no-cache-dir "vllm==0.8.5.post1" "torch==2.6.0" "torchvision==0.21.0" "torchaudio==2.6.0" "tensordict==0.6.2" torchdata

echo "2. install basic packages"
pip install -i $PIP_INDEX_URL "transformers[hf_xet]>=4.51.0" accelerate datasets peft hf-transfer \
...
pip install -i $PIP_INDEX_URL opencv-python
pip install -i $PIP_INDEX_URL opencv-fixer && \
    python -c "from opencv_fixer import AutoFix; AutoFix()"


if [ $USE_MEGATRON -eq 1 ]; then
    echo "6. Install cudnn python package (avoid being overrided)"
    pip install -i $PIP_INDEX_URL nvidia-cudnn-cu12==9.8.0.87
fi
