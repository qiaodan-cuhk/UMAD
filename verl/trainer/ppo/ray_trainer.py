...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        # 新增记录原始问题index和reward，用于统计不同模型的推理边界
        sample_indexs = []
        ...
        for test_data in self.val_dataloader:
            ...
            indices = test_batch.non_tensor_batch.get("index", [0] * len(scores))
        ...
        # 新增：记录原始问题index和reward
        val_data_index_score = [{"index": idx, "score": score} for idx, score in zip(sample_indexs, sample_scores)]
        file_path = os.path.join(val_data_dir, f"val_index_score_{self.global_steps}.csv")
        print(f"save val index score csv in {file_path}")
        with open(file_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["index", "score"])
            for row in val_data_index_score:
                writer.writerow([row["index"], row["score"]])
