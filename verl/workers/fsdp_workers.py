...
class ActorRolloutRefWorker(Worker):
    ...
    # 新增confidence选项，只有compute log prob需要，compute ref log prob不需要
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                output, entropys, confidences = self.actor.compute_log_prob(data=data, calculate_entropy=True, calculate_confidence=calculate_confidence)
            output = DataProto.from_dict(
                tensors={"old_log_probs": output, "entropys": entropys, "confidences": confidences},
    ...
    # 专门用于计算confidence的验证
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_metrics_for_confidence(self, data: DataProto):
        # when is_lora is True, we use the actor without lora applied to calculate the log_prob
        # which is mostly used for ref log_prob calculation
        assert self._is_actor
        if self._is_offload_param:
    ...
    def compute_ref_log_prob(self, data: DataProto):
        if self._is_lora:
            ...
            data = self.compute_log_prob(data)  # old_log_probs": output, "entropys": entropys, "confidences": 
            # this old_log_probs is in fact ref_log_prob
            data = DataProto.from_dict(tensors={'ref_log_prob': data.batch['old_log_probs'], 'ref_confidences': data.batch['confidences']})
        ...
        with self.ulysses_sharding_manager:
            ...
            # ref policy不需要计算confidence，这里算一下是为了额外统计一个metric做对比
            output, _, ref_confidences= self.ref_policy.compute_log_prob(data=data, calculate_entropy=False, calculate_confidence=True)
            output = DataProto.from_dict(tensors={"ref_log_prob": output, "ref_confidences": ref_confidences})
            output = self.ulysses_sharding_manager.postprocess_data(output)
